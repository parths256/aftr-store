...
renderBag();
/* =========================================
   AFTR CAMPAIGN SLIDESHOW
========================================= */

const campaignImages = [
  "/static/product 10.jpg",
  "/static/product 9.jpg",
  "/static/product 8.jpg",
  "/static/product 7.jpg",
  "/static/product 6.jpg"
];
...
if (campaignImage) {
  ...
  // Preload images so transitions stay smooth
  campaignImages.forEach(src => {
    const img = new Image();
    img.src = src;
  });
  ...
  setInterval(() => {
    ...
    campaignImage.classList.add("changing");

    setTimeout(() => {

      campaignIndex =
        (campaignIndex + 1) % campaignImages.length;

      campaignImage.src =
        campaignImages[campaignIndex];

      if (campaignNumber) {
        campaignNumber.textContent =
          `AFTR / ${String(campaignIndex + 1).padStart(2, "0")}`;
      }

      campaignImage.classList.remove("changing");
